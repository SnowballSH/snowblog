...
export const httpRequestsTotal = new client.Counter({
	name: 'snowblog_admin_http_requests_total',
	help: 'Requests served by the snowblog admin dashboard',
	labelNames: ['method', 'status', 'route'] as const,
	registers: [registry]
});

export const httpRequestDuration = new client.Histogram({
	name: 'snowblog_admin_http_request_duration_seconds',
	help: 'Request duration for the snowblog admin dashboard',
...
export const upstreamFailures = new client.Counter({
	name: 'snowblog_admin_upstream_failures_total',
...
export function startMetricsServer(listen: string): Server {
	...
	if (!host || !Number.isInteger(port) || port < 1 || port > 65535) {
		throw new Error(`invalid SNOWBLOG_ADMIN_METRICS_LISTEN: ${listen}`);
	}
